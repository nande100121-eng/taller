...
export const Navbar: React.FC = () => {
  ...
  const erpLinks = [
    { name: "Portería & Semáforo", href: "/dashboard/porteria", icon: ShieldAlert, color: "text-red-400" },
    { name: "Recepción & Citas", href: "/dashboard/recepcion", icon: Calendar, color: "text-blue-400" },
    { name: "Taller & Kanban", href: "/dashboard/taller", icon: Wrench, color: "text-amber-400" },
    { name: "Almacén & Herramientas", href: "/dashboard/almacen", icon: Package, color: "text-emerald-400" },
    { name: "Caja & Facturación", href: "/dashboard/caja", icon: CreditCard, color: "text-purple-400" },
    { name: "Certificaciones GNV/GLP", href: "/dashboard/certificaciones", icon: Award, color: "text-teal-400" },
    { name: "Asistencia Biométrica", href: "/dashboard/asistencia", icon: Clock, color: "text-cyan-400" },
    { name: "Admin CMS Web", href: "/dashboard/admin/cms", icon: Settings, color: "text-reygas-red" },
    { name: "Tablas Maestras Inline", href: "/dashboard/admin/tables", icon: Table, color: "text-indigo-400" },
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          <nav className="hidden md:flex items-center space-x-6">
            ...
            {/* If logged in, show ERP Stations Dropdown */}
            {isAuthenticated ? (
            ...
            {/* Admin Live Editing Toggle Button (Visible only when Admin logged in) */}
            {isAuthenticated && userRole === "admin" && (
              <button
                onClick={toggleVisualEditing}
                className={`flex items-center gap-2 px-3.5 py-2 rounded-full text-xs font-bold transition-all shadow-lg border ${
                  isVisualEditing
                    ? "bg-reygas-red text-white border-reygas-redBright shadow-reygas-red/40 animate-pulse"
                    : "bg-gray-800 text-gray-400 border-gray-700 hover:text-white"
                }`}
              >
                <Edit3 className="w-3.5 h-3.5" />
                <span>{isVisualEditing ? "Edición Web Activa" : "Activar Edición Web"}</span>
              </button>
            )}

            {/* User Session & Login/Logout Link */}
            {isAuthenticated ? (
              ...
              >
                <Lock className="w-3.5 h-3.5 text-reygas-red group-hover:text-white" />
          ...
          <div className="md:hidden flex items-center gap-3">
            {isAuthenticated ? (
              <button
                ...
                className="p-2 rounded-lg bg-red-950/40 text-red-400 text-xs font-bold"
              >
                Salir
              </button>
            ) : (
