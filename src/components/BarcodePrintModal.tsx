      ...
      <div className="glass-panel bg-reygas-dark/95 border border-white/15 rounded-3xl p-4 sm:p-6 max-w-6xl w-full shadow-2xl shadow-black/95 space-y-6 my-auto max-h-[94vh] flex flex-col">
            ...
            <div>
              <h3 className="text-xl font-black text-white flex items-center gap-2">
                ...
                <span className="text-[11px] font-mono px-2 py-0.5 rounded-full bg-amber-500/20 text-amber-300 border border-amber-500/30">
                  8 por Hoja A4 • Por Letra
                </span>
              </h3>
              <p className="text-xs text-gray-400">
                Diseño limpio con espacio para foto/imagen, nombre, marca, serie y código de barras SKU.
              </p>
            </div>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 overflow-y-auto flex-1 pr-1">
          ...
          <div className="lg:col-span-5 space-y-5">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 space-y-2">
                    ...
                    <p className="text-[10px] text-gray-400">
                      Esta imagen aparecerá en el recuadro izquierdo de cada una de las 8 etiquetas.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="p-4 rounded-2xl bg-blue-950/30 border border-blue-500/30 space-y-2 text-xs">
              ...
              <p className="text-[11px] text-gray-300 pt-1 border-t border-white/10 leading-relaxed">
                ✓ <strong>Separación por Letra:</strong> Cada letra inicial (A, B, C...) se imprime en una hoja independiente. Si una letra tiene 3 productos, la siguiente letra comenzará en una nueva hoja.
              </p>
            </div>
          </div>
          ...
          <div className="lg:col-span-7 space-y-4 flex flex-col">
              ...
              <div className="flex items-center gap-2">
                ...
                {currentPage && (
                  <span className="px-2 py-0.5 rounded-lg bg-black/60 text-amber-400 font-mono font-bold text-xs border border-amber-500/30">
                    Letra: {currentPage.letter} ({currentPage.items.length} de 8)
                  </span>
                )}
              </div>
            ...
            <div className="flex-1 bg-gray-300 p-3 sm:p-4 rounded-2xl border border-white/20 overflow-y-auto max-h-[520px] flex items-center justify-center">
              {currentPage && currentPage.items.length > 0 ? (
                <div className="bg-white text-black w-full max-w-[500px] aspect-[1/1.414] p-3 rounded-lg shadow-2xl flex flex-col justify-between border border-gray-400">
                  ...
                  <div className="grid grid-cols-2 grid-rows-4 gap-2 flex-1">
                    {currentPage.items.map((item, idx) => (
                      <div
                        key={`${item.id}-${idx}`}
                        className="border border-dashed border-gray-400 p-2 rounded-lg bg-white flex gap-2 items-center overflow-hidden shadow-sm"
                      >
                        {/* Espacio para Imagen del Producto */}
                        <div className="w-16 h-16 sm:w-20 sm:h-20 border border-gray-300 rounded-md bg-gray-50 flex items-center justify-center p-1 shrink-0 overflow-hidden">
                          {productImageUrl ? (
                            <img
                              src={productImageUrl}
                              alt={item.name}
                        ...
                        <div className="flex-1 min-w-0 flex flex-col justify-between h-full py-0.5">
                          <div>
                            {/* Nombre del Producto */}
                            <h4 className="text-[9px] font-black text-black leading-tight line-clamp-2 uppercase">
                              {item.name}
                            </h4>
                            {/* Marca y Serie */}
                            <p className="text-[7.5px] text-gray-600 font-semibold truncate mt-0.5">
                    ...
                    {Array.from({ length: 8 - currentPage.items.length }).map((_, emptyIdx) => (
                      <div
                        key={`empty-${emptyIdx}`}
                        className="border border-dotted border-gray-300 p-2 rounded-lg bg-gray-50/60 flex items-center justify-center text-[8px] text-gray-400 font-mono"
                      >
                        [ Espacio Disponible ]
                      </div>
                    ))}
